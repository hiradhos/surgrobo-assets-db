...
% Enter title:
\title{NETTER-DB: A Public Multimodal Database of Surgical Robotic and Human Anatomical Assets}
...
{USC Keck School of Medicine, March 2026}
...
As medicine ventures into the Age of AI, there will be a burgeoning demand for various clinical assets, including:
\begin{itemize}
\item 3D digital replicas of human anatomy, surgical robots, surgical equipment, and clinical infrastructure
\item Surgical video footage
\item Surgical robotic kinematics data 
\item Simulation and training assets
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Motivation}
Although many academic labs and medical centers have stepped in to meet this demand, there does not appear to exist a \textsb{single comprehensive and unified public database to allow for easy search and retrieval of these assets}.
\end{frame}

\begin{frame}
\frametitle{Motivation}
Several disciplines--including structural bioinformatics (Protein Data Bank, UniProt), genetics (GenBank, NCBI), medical imaging (TCIA) and oncology (TCGA, NCDB)--have demonstrated that building a single comprehensive database is feasible and can have immense utility.
\end{frame}

\begin{frame}
\frametitle{Success Story: The Protein Data Bank}
\includegraphics[width=0.5\textwidth]{figures/pdb_logo.jpg}
\includegraphics[width=0.8\textwidth]{figures/pdb_stats.png}
\end{frame}

\begin{frame}
\frametitle{Database Overview}
\centering
\includegraphics[width=\textwidth,height=0.9\textheight,keepaspectratio]{figures/backend.png}
\end{frame}

\begin{frame}
\frametitle{Database Overview}
\centering
\includegraphics[width=\textwidth,height=0.6\textheight,keepaspectratio]{figures/frontend.png}
\end{frame}

\begin{frame}{The Road Ahead}
\begin{enumerate}[<+-| alert@+>]
    \item Manual literature review (3D assets, CT/MRI images, segmentation models, surgical video footage, kinematics)
    \item Manually populate DB with unusual assets/improve automated scraping pipeline
    \item Set up medical image $\rightarrow$ 3D synthetic asset pipeline
...
\begin{itemize}[<+-| alert@+>]
    \item How can we automate scraping such that realistic \& relevant assets populate the DB without excessive filtering by the LLM?
    \item What assets should we store internally (via S3 bucket) and what should point to the source URL?
    \item How many synthetic 3D assets should we generate, and which types would be most useful?
    \item Should we incorporate LLM-directed database retrieval?
    \item Would it be feasible to let users upload CT/MRI scans to generate 3D assets using our automated processing pipeline?
\end{itemize}
\end{frame}
